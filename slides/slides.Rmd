---
title: "Extend The Functionality Of Your R Markdown"
...
.small[https://pkgs.rstudio.com/rmarkdown/]
]
.pull-right.center[
...
.center[![:scale 70%, Diagram showing the different step of rendering, using knitr first from Rmd to md, then Pandoc from md to output format like HTML, DOC and PDF (using LaTeX)](https://bookdown.org/yihui/rmarkdown-cookbook/images/workflow.png)]
.source-fig[
...
```
````
There are **knitr** options
* `include = FALSE`: Chunk will be evaluated but no output or source included.
* `echo = FALSE`: Do not show output the source as code block. Set globally here.
...
````markdown
```{r, include = FALSE}`r ''`
island_repart_alt <- "Horizontal bar chart, faceted by Species
showing the number of penguins living on each island, Torgersen, 
Dream and Biscoe. Adelie lives on the three, whereas Gentoo on 
Biscoe and Chinstrap on Dream."
```
...
.box[
How to improve our highlighted results ?</br>
...
This would be the CSS way: Can we avoid repetition ?
